...
/**
 * The form type for voting in an election run-off.
 */
class VotingRunOffType extends AbstractType
{
    /**
    ...
    {
        $election = $elections->getOpenElectionRunOff();
        if ($election) {
            // if there isn't an open election, this form type should never be instantiated
            $this->candidates = $candidates->getRunOffCandidatesByYear($election->getYear());
        }
    }
